//! Horizontally scrollable `Row`: 20 card buttons panned left/right by drag.
//!
//! The same LVGL-style scroll surface as `scroll_list`, but on the horizontal
//! axis: a 1:1 drag pans the row, a tap picks a card, a fling coasts with
//! friction, and over-dragging an edge stretches then springs back. Scroll
//! state is owned by the host, mutated only in `update`, and momentum is driven
//! by a self-rescheduling [`tick_task`].
...
impl ScreenView<Rgb565, Msg> for Screen {
    ...
    fn view(&self) -> Element<'_, Rgb565, Msg> {
        let label: String = match self.picked {
            Some(i) => format!("Picked card {i}"),
            None => "Drag sideways to scroll, tap to pick".into(),
        };
...
async fn main(_spawner: embassy_executor::Spawner) {
    zest::run::<App>("zest - Scroll Row").await;
}
