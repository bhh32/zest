...
//! `iso_8859_15` bitmap fonts (grid-perfect, identical to the originals);
//! symbols not in those fonts (`⌫`, `←`/`→`/`↑`/`↓`, `↵`, `✓`/`✗`,
//! checkboxes, geometric shapes, light box-drawing) are rasterized from
//! DejaVu Sans Mono. Three sizes (caption/body/heading) share one glyph set
//! of 240 code points. Regenerate with `tools/fontgen`.
...
const GLYPHS: StrGlyphMapping = StrGlyphMapping::new(" !\"#$%&'()*+,-./0123456789:;<=>?@ABCDEFGHIJKLMNOPQRSTUVWXYZ[\\]^_`abcdefghijklmnopqrstuvwxyz{|}~ ¡¢£¤¥¦§¨©ª«¬­®¯°±²³´µ¶·¸¹º»¼½¾¿ÀÁÂÃÄÅÆÇÈÉÊËÌÍÎÏÐÑÒÓÔÕÖ×ØÙÚÛÜÝÞßàáâãäåæçèéêëìíîïðñòóôõö÷øùúûüýþÿ‐‑‒–—‘’“”†‡•…‹›€℗™←↑→↓↵⌫─│┌┐└┘├┤┬┴┼■□▲▶▼◀○●★☆☐☑✓✗", 31);

/// Zest caption mono font (6x10): grid-perfect bitmap text + symbol glyphs.
...
/// Zest default body mono font (8x13): grid-perfect bitmap text + symbol glyphs.
...
/// Zest heading mono font (10x20): grid-perfect bitmap text + symbol glyphs.
